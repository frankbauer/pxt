...
let ifacecpp: pxt.worker.Iface

namespace pxt.clocks {
    ...
    function compileFunctionCall(e: Environment, b: Blockly.Block, comments: string[]): JsNode {
        ...
        const args: pxt.blocks.BlockParameter[] = (b as Blockly.FunctionCallBlock).getArguments().map(a => {
            return {
                actualName: a.name,
                definitionName: a .id
            };
    ...
    function compileArgument(e: Environment, b: Blockly.Block, p: pxt.blocks.BlockParameter, comments: string[], beginningOfStatement = false): JsNode {
    ...
    function compileStdCall(e: Environment, b: Blockly.Block, func: StdFunc, comments: string[]): JsNode {
        let args: JsNode[]
        if (isMutatingBlock(b) && b.mutation.getMutationType() === pxt.blocks.MutatorTypes.RestParameterMutator) {
        ...
        if (func.isIdentity)
            ...
            if (func.attrs.defaultInstance) {
                let instance: JsNode;
                if (isMutatingBlock(b) && b.mutation.getMutationType() === pxt.blocks.MutatorTypes.DefaultInstanceMutator) {
    ...
    function compileEvent(e: Environment, b: Blockly.Block, stdfun: StdFunc, args: string[], ns: string, comments: string[]): JsNode {
        ...
        if (isMutatingBlock(b) && b.mutation.getMutationType() === pxt.blocks.MutatorTypes.ObjectDestructuringMutator) {
    ...
    function isMutatingBlock(b: Blockly.Block): b is pxt.blocks.MutatingBlock {
        return !!(b as pxt.blocks.MutatingBlock).mutation;
    }
    ...
    function countOptionals(b: Blockly.Block) {
        if ((b as pxt.blocks.MutatingBlock).mutationToDom) {
            const el = (b as pxt.blocks.MutatingBlock).mutationToDom();
